...
**Shareable link is automatic and permanent, confirmed 2026-07-18:** the PR comment includes a working `?_vercel_share=<token>` link directly, no manual dashboard step needed. This works because Vercel's Shareable Link bypass (`PATCH /v1/aliases/{id}/protection-bypass`, confirmed via direct API testing — not part of any documented CLI command) is tied to the **alias itself** (`dev.coastalgrannys.com`), not to whichever specific deployment currently sits behind it — calling the create endpoint again returns `protection_bypass_conflict: shareable link already exists`, and the original token generated weeks/deployments ago was directly verified (via `curl -L` following the full redirect/cookie chain) to still work after the alias had since been re-pointed to several different PR deployments. So one token, generated once, keeps working across every future `deploy-dev` build — it's stored as the `VERCEL_SHARE_TOKEN` repo secret and the workflow just references it, rather than calling the Vercel API on every run. It would only need regenerating if someone explicitly revokes it from Vercel's Share UI (`revoke` + `regenerate` in the same API's request body) — if that ever happens, update the `VERCEL_SHARE_TOKEN` secret with the new value.

Requires four repository secrets (Settings → Secrets and variables → Actions, already set as of 2026-07-18): `VERCEL_TOKEN` (a dedicated token from https://vercel.com/account/tokens — the CLI session used for everything else in this doc is authenticated via a scoped plugin integration that can't mint new tokens itself, so this one has to be created manually), `VERCEL_ORG_ID` (`team_YvNfQhQsyoyzD4qgK6VEzZQU`), `VERCEL_PROJECT_ID` (`prj_oXOMYkESfxLZexOn1H3kHuRyevK7`), `VERCEL_SHARE_TOKEN` (the permanent shareable-link token described above). These are CI-only — the app itself never reads any of them, so they don't belong in `.env.local`.
